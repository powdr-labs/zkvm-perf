...
machine Brainfuck {
	...
	function main {
		...
		read_program_loop:
			...
			mstore CNT + 0 /*PROGRAM_START*/, ${ Query::Input(std::convert::int(std::prover::eval(CNT)) + 1) };
			CNT <=X= CNT + 1;
		...
		read_input:
			CNT <=X= 0;
			// read input length
			in_ptr <=X= ${ Query::Input(std::convert::int(std::prover::eval(A)) + 1) };
		read_input_loop:
			branch_if_zero in_ptr - CNT, end_read_input;
			mstore CNT + 10000 /*INPUT_START*/, ${ Query::Input(std::convert::int(std::prover::eval(CNT) + std::prover::eval(A)) + 2) };
			CNT <=X= CNT + 1;
      ...
      in_ptr <=X= in_ptr + 1;
		...
		routine_write:
			A <== mload(dp);
			A <=X= ${ Query::Output(1, std::convert::int(std::prover::eval(A))) };
			A <== jump(end_run_op);
		// ==== end of `.` helper routine
